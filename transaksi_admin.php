                ...
                <div class="container-fluid px-4">
                    ...
                    <div class="container">
                        <div class="row">
                            ...
                            // Query untuk mengambil data dari tabel stand dan menghitung total pesanan
                            $sql = "SELECT stand.id, stand.Pemilik, stand.Kontak, COUNT(transasksi.id_stand) AS total_pesanan, SUM(transasksi.total) AS total_harga
                            FROM stand
                            LEFT JOIN transasksi ON stand.id = transasksi.id_stand
                            GROUP BY stand.id";
                            ...
                            if (mysqli_num_rows($result) > 0) {
                                ...
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ...
                                    echo '<h5 class="card-title">Stand ' . $row['id'] . '</h5>';
                                    echo '<p class="card-text">Pemilik Stand: ' . $row['Pemilik'] . '</p>';
                                    echo '<p class="card-text">Kontak Pemilik: ' . $row['Kontak'] . '</p>';
                                    ...
                                    echo '<a href="#" class="btn btn-primary d-flex justify-content-center" data-bs-toggle="modal" data-bs-target="#StandModal' . $row['id'] . '">Detail Transaksi</a>';
                                    echo '</div>';
                                    echo '</div>';
                                    echo '</div>';
                                    // Tambahkan modal dengan id yang sesuai
                                    echo '<div class="modal fade" id="StandModal' . $row['id'] . '" tabindex="-1" aria-labelledby="StandModal' . $row['id'] . '" aria-hidden="true">';
                                    echo '<div class="modal-dialog">';
                                    echo '<div class="modal-content">';
                                    echo '<div class="modal-header">';
                                    echo '<h5 class="modal-title" id="exampleModalLabel">Stand ' . $row['id'] . ' </h5>';
                                    ...
                                    $sql_transaksi = "SELECT id_user, SUM(count) AS count, SUM(total) AS total, SUM(np) AS np FROM transasksi WHERE id_stand = " . $row['id'] . " GROUP BY id_user ORDER BY np ASC";
                                    ...
                                    if (mysqli_num_rows($result_transaksi) > 0) {
                                        while ($row_transaksi = mysqli_fetch_assoc($result_transaksi)) {
                                            ...
                                            echo '<td>' . $row_transaksi['count'] . '</td>';
